...
const AddButton: React.FC<AddButtonProps> = ({ className, ...props }) => {
    return (
        <button
            className={cn('h-[40px] w-[40px] flex items-center justify-center rounded-full transition-all disabled:pointer-events-none disabled:opacity-40 shrink-0 outline-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] aria-invalid:ring-destructive/20 aria-invalid:border-destructive bg-purple text-foreground hover:bg-purple-dark cursor-pointer', className)}
            {...props}
        >
